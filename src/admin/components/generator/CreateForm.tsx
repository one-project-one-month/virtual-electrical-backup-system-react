...
const CreateForm = () => {
  ...
  const { register, handleSubmit,control, formState: { errors, isSubmitting }, setValue, getValues } = useForm<Generator>({
    resolver: zodResolver(formSchema),
  });
  const formSubmit: SubmitHandler<Generator> = async (data) => {
    await new Promise((resolve) => setTimeout(resolve, 2000)); // Simulate loading state
    console.log(data);
  };
